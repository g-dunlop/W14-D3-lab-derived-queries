...
//    }
    ...
    public ResponseEntity<List<Ship>> findShipsThatHavePiratesNamedQueryString(
            @RequestParam(name="piratesfirstname", required = false) String piratesFirstName){
        if (piratesFirstName != null){
            return new ResponseEntity<>(shipRepository.findByPiratesFirstName(piratesFirstName), HttpStatus.OK);
        }
        return new ResponseEntity<>(shipRepository.findAll(), HttpStatus.OK);
    }
}
